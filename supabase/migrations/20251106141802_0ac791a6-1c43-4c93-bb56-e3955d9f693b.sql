-- Fix unsafe DELETE by using TRUNCATE in rebuild_category_search_index
...
AS $$
BEGIN
  -- Clear existing index quickly and safely; TRUNCATE avoids RLS DELETE policies and WHERE enforcement
  TRUNCATE TABLE public.category_search_index;
